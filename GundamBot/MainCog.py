...
class MainCog(commands.Cog, name='General'):
    def __init__(self, bot):
        ...
        print("GundamBot is now running!")
    ...
    # !gundam random
    random_HELP = 'Links a random Gundam within channel'
    ...
    async def randomGundam(self, ctx):
        ...
        tag = soup.find("img",  class_="pi-image-thumbnail")
        imgURL = tag['src']
        async with aiohttp.ClientSession() as session:
            async with session.get(imgURL) as resp:
                ...
                await ctx.send(response,file=discord.File(data, 'guessthatgundam.png'))

    @randomGundam.error
    async def randomGundam_error(self, ctx, error):
        await ctx.send('Sorry, the random command is not working right now. Contact cherry#1048 for help!')
    ...
    @guessThatGundam.error
    async def guessThatGundam_error(self, ctx, error):
    ...
    @guessThatCharacter.error
    async def guessThatCharacter_error(self, ctx, error):
        await ctx.send('Sorry, Guess that Gundam Character is not working right now. Contact cherry#1048 for help!')
